...
| 패키지                   | 이전 버전 | 최신 버전 | 상태        |
| ------------------------ | --------- | --------- | ----------- |
| openai                   | 1.10.0    | 1.109.1   | ✅ 업데이트 |
| httpx                    | 0.27.0    | 0.28.1    | ✅ 업데이트 |
| tweepy                   | 4.16.0    | 4.16.0    | ⚪ 최신     |
| pandas                   | 2.3.1     | 2.3.2     | ✅ 업데이트 |
| tiktoken                 | 0.5.2     | 0.11.0    | ✅ 업데이트 |
| matplotlib               | 3.10.5    | 3.10.6    | ✅ 업데이트 |
| plotly                   | 6.2.0     | 6.3.0     | ✅ 업데이트 |
| scipy                    | 1.16.1    | 1.16.2    | ✅ 업데이트 |
| scikit-learn             | 1.7.1     | 1.7.2     | ✅ 업데이트 |
| langchain                | 0.1.4     | 0.3.27    | ✅ 업데이트 |
| langchain-core           | 0.1.17    | 0.3.76    | ✅ 업데이트 |
| langchain-community      | 0.0.17    | 0.3.30    | ✅ 업데이트 |
| langchain-openai         | 0.0.5     | 0.3.33    | ✅ 업데이트 |
| langsmith                | 0.0.92    | 0.4.31    | ✅ 업데이트 |
| google-api-python-client | 2.177.0   | 2.183.0   | ✅ 업데이트 |
| pypdf                    | 5.9.0     | 6.1.0     | ✅ 업데이트 |
| chromadb                 | 1.0.15    | 1.1.0     | ✅ 업데이트 |
| pydantic                 | 2.11.7    | 2.11.9    | ✅ 업데이트 |
| numpy                    | 1.26.4    | 2.3.3     | ✅ 업데이트 |
| pillow                   | 11.3.0    | 11.3.0    | ⚪ 최신     |

> 💡 **설치 방법**: `pip install -r requirements.txt` 명령어를 실행하시면 모든 패키지가 최신 버전으로 설치됩니다.
...
> ⚠️ **주의사항**: 일부 패키지(특히 langchain 관련 패키지들과 numpy)는 메이저 버전이 크게 업데이트되어 기존 코드와 호환성 문제가 발생할 수 있습니다. 업데이트 후에는 애플리케이션을 테스트해보시는 것을 권장합니다.

Google API 키 설정 - 실제 Google API 키와 Custom Search Engine ID가 필요합니다.
🔧 다음 단계:
Google API 키 발급 방법:
Google Cloud Console (https://console.cloud.google.com/) 접속
API 및 서비스 → 라이브러리에서 "Custom Search API" 활성화
사용자 인증 정보 → API 키 생성
Custom Search Engine (https://cse.google.com/)에서 검색 엔진 생성
.env 파일의 your_google_api_key_here와 your_custom_search_engine_id_here를 실제 값으로 교체

# 《OpenAI API와 파이썬으로 나만의 챗GPT 만들기》 실습 예제

원서 제목: ChatGPT API×Python で始める対話型 AI 実装入門（GPT-3.5&GPT-4 対応）
